...
class DetailActivity : AppCompatActivity() {
    ...
    private fun showDetailMovie(detailMovie: Movie?) {
        detailMovie?.let {
            ...
            var statusFavorite = detailViewModel.isExistInFavoriteMovie(detailMovie.movieId)
            setStatusFavorite(statusFavorite)
            binding.detailFavBtn.setOnClickListener {
                if (statusFavorite){
                    detailViewModel.deleteFavoriteMovie(detailMovie)
                    Log.d("Detail Activity", "Delete Favorite Movie")
                }
                else{
                    detailViewModel.insertFavoriteMovie(detailMovie)
                    Log.d("Detail Activity", "Insert Favorite Movie")
                }
    ...
    private fun showDetailTvShow(detailTvShow: TvShow?) {
        detailTvShow?.let {
            ...
            var statusFavorite = detailViewModel.isExistInFavoriteTvShow(detailTvShow.tvShowId)
            setStatusFavorite(statusFavorite)
            binding.detailFavBtn.setOnClickListener {
                if (statusFavorite){
                    detailViewModel.deleteFavoriteTvShow(detailTvShow)
                    Log.d("Detail Activity", "Delete Favorite TvShow")
                }else{
                    detailViewModel.insertFavoriteTvShow(detailTvShow)
                    Log.d("Detail Activity", "Insert Favorite TvShow")
                }
                setStatusFavorite(!statusFavorite)
            }
            binding.detailPb.visibility = View.GONE
